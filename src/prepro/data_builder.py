...
def load_json(p, lower):
    ...
    tgt = []
    flag = False
    for sent in json.load(open(p,'rb'))['sentences']:
...
def _format_to_bert(params):
    ...
    jobs = json.load(open(json_file,'rb',))
    datasets = []
    for d in jobs:
...
def format_to_lines(args):
    ...
    for corpus_type in ['valid', 'test', 'train']:
        temp = []
        # for line in open(pjoin(args.map_path, 'arabic_mapping_' + corpus_type + '.txt')):
        for line in open(pjoin(args.map_path, 'arabic_TestData_mapping'+ '.txt')):
    ...
    for f in glob.glob(pjoin(args.raw_path, '*.json')):
        ...
        # print(real_name in corpus_mapping['test'])
        # if (real_name in corpus_mapping['valid']):
            # valid_files.append(f)
        # elif (real_name in corpus_mapping['test']):
            # test_files.append(f)
...
def format_xsum_to_lines(args):
    ...
    corpus_mapping = json.load(open(pjoin(args.raw_path, 'XSum-TRAINING-DEV-TEST-SPLIT-90-5-5.json'),'rb'))
    ...
    for corpus_type in datasets:
        ...
        for d in pool.imap_unordered(_format_xsum_to_lines, a_lst):
            ...
            if (len(dataset) > args.shard_size):
                pt_file = "{:s}.{:s}.{:d}.json".format(args.save_path, corpus_type, p_ct)
                with open(pt_file, 'w','rb') as save:
                    save.write(json.dumps(dataset))
                    p_ct += 1
                    dataset = []
